...
export function renderSettings(container) {
  ...
  container.innerHTML = `
    <div class="page settings-page">
        ...
        <div>
          ...
          <div style="display: flex; flex-wrap: wrap; gap: var(--space-sm)">
            ${avatars
              .map(
                (a) => `
              <button class="emoji-btn avatar-pick ${a === data.profile.avatar ? "active" : ""}" data-avatar="${a}" 
                style="${a === data.profile.avatar ? "border: 2px solid var(--neon-purple); box-shadow: var(--shadow-neon-purple)" : ""}">
                ${a}
              </button>
            `,
              )
      ...
      <div style="text-align: center; padding: var(--space-2xl) 0; color: var(--text-tertiary); font-size: var(--text-xs)">
        InfiniToe v1.0.0 • Made with 💜
      </div>
    </div>
  `;
  ...
  document.getElementById("btn-reset")?.addEventListener("click", () => {
    if (confirm("Are you sure? This cannot be undone!")) {
      ...
      showToast("All data reset!", "🗑️", 2000);
