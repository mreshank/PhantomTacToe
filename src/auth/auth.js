...
export function getClerkUser() { return clerkInstance?.user || null; }
export function isAuthAvailable() { return !!clerkInstance; }
export function isSignedIn() { return !!clerkInstance?.user; }
...
export function getClerkProfile() {
  ...
  return {
    id: user.id,
    name: user.username || user.firstName || user.fullName || user.primaryEmailAddress?.emailAddress?.split("@")[0] || "Player",
...
export async function signIn() { 
  if (clerkInstance) { 
    try { await clerkInstance.openSignIn({}); } catch(err){}
  }
}
export async function signUp() {
  if(clerkInstance) {
    try { await clerkInstance.openSignUp({}); } catch(err){}
  }
}
export async function signOut() {
  if (clerkInstance) {
    try { await clerkInstance.signOut(); notifyListeners(); } catch (err) {}
  }
}
